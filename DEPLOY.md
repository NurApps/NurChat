...
- **Postgres free умирает через 90 дней вместе с данными.** Календарное
  напоминание на день 80: `pg_dump` → новый инстанс. Аккаунты иначе
  сгорят квартально.
- **Диск эфемерный:** `media/` (фото, файлы, аватары) исчезает при каждом
  рестарте/редеплое. Переписка (в БД) цела, вложения — нет.
