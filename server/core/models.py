...
class User(Base):
    ...
    status = Column(String, default="", nullable=True)
...
class Chat(Base):
    ...
    disappears_after_seconds = Column(Integer, default=0)
    group_key = Column(Text, nullable=True)
    group_key_creator_id = Column(String, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class Contact(Base):
    ...
    id = Column(String, primary_key=True, index=True)
    user_id = Column(String, ForeignKey("users.id", ondelete="CASCADE"))
    contact_user_id = Column(String, ForeignKey("users.id", ondelete="CASCADE"))
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class GroupInvite(Base):
    ...
    invitee_id = Column(String, ForeignKey("users.id", ondelete="CASCADE"))
...
class BlockedUser(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(String, ForeignKey("users.id", ondelete="CASCADE"))
    blocked_user_id = Column(String, ForeignKey("users.id", ondelete="CASCADE"))
    created_at = Column(DateTime(timezone=True), server_default=func.now())
