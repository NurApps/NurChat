...
В `.env` добавьте (или позвольте серверу сгенерировать автоматически — не для продакшена, см. `AGENTS.md`):
...
Все запросы требуют `Authorization: Bearer <token>` (кроме `/auth/login`).
...
**Поля:**
- `secret`: секрет для ручного ввода в приложении аутентификации
- `uri`: `otpauth://` URI (то же самое, что закодировано в `qr_code`)
- `qr_code`: Data URI с QR-кодом для сканирования
- `backup_codes`: 10 кодов, показываются **только один раз**

На этом этапе 2FA ещё НЕ включена — секрет сохранён на сервере, ждёт подтверждения.
...
```json
{
  ...
  "refresh_token": "<2fa-pending token>",
  "token_type": "bearer",
  "user": { "...": "..." },
  "requires_2fa": true
}
...
максимум 5 попыток в минуту — `@limiter.limit("5/minute")`).
