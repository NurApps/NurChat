...
class TestPolls:
    ...
    def test_create_poll(self, client, user_a, user_b):
    ...
    def test_vote_poll(self, client, user_a, user_b):
        ...
        resp = client.post(
            f"/api/chat/polls/{poll['id']}/vote",
            json={"option_ids": [option_id]},
            headers=auth(user_b["token"]),
        )
        assert resp.status_code == 200, f"Vote failed: {resp.status_code} {resp.json()}"
        assert resp.json()["total_votes"] == 1

    def test_vote_own_poll(self, client, user_a, user_b):
        chat_id = self._create_chat(client, user_a, user_b)
        resp = client.post(
            f"/api/chat/chats/{chat_id}/polls",
            json={
    ...
    def test_cannot_vote_multiple_without_flag(self, client, user_a, user_b):
        chat_id = self._create_chat(client, user_a, user_b)
        resp = client.post(
            f"/api/chat/chats/{chat_id}/polls",
            json={
        ...
        resp = client.post(
            f"/api/chat/polls/{poll['id']}/vote",
            json={"option_ids": opts},
            headers=auth(user_b["token"]),
        )
        assert resp.status_code == 400

    def test_get_polls(self, client, user_a, user_b):
        chat_id = self._create_chat(client, user_a, user_b)
        client.post(
            f"/api/chat/chats/{chat_id}/polls",
            json={
