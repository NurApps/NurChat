...
Постоянного бесплатного ХОСТИНГА ПРИЛОЖЕНИЯ **не существует**:
всё бесплатное либо засыпает, либо с плавающим адресом (quick tunnel),
либо требует карту. Render для бэкенда не тянет (сон, эфемерный диск,
смерть Postgres через 90 дней, нет TURN) — убран из схемы полностью.
Поэтому при бюджете 0 работаем так:

**База (постоянно, $0):** домашний ПК / старый телефон + quick tunnel
(раздел 0.1). Друг привыкает, что адрес иногда меняется.

**Облачная БД ($0, вечная): Supabase free.** А вот база может жить
в облаке бесплатно и вечно: 500 MB Postgres без срока годности.
Релей (хоть дома, хоть на триале) ходит в неё по `DATABASE_URL`:

1. supabase.com → New project (free, карты не надо) → ждать готовности.
2. Project Settings → Database → Connection string (URI), пароль
   URL-encode (`@` → `%40` и т.д.).
3. В `.env` релея: `DATABASE_URL=postgresql://postgres:...@db.XXX.supabase.co:5432/postgres`
4. Первый старт сам создаст таблицы (`alembic upgrade head` в lifespan).
   Пауза Supabase после 7 дней тишины не страшна: любой запрос будит,
   а наши клиенты дергают релей постоянно.

Профит: данные больше не привязаны к железу. Сдох домашний ПК, кончился
триал — поднял релей в новом месте с тем же `DATABASE_URL`, и все
аккаунты/история на месте. Копировать `nurchat.db` больше не надо
(только `media/`, если важны старые вложения).

Честные ограничения связки:
- **Файлы живут на диске релея** (`media/` локально). Сдох хост — вложения
  умерли, переписка цела. Вечное хранение файлов — следующий шаг,
  Supabase Storage (1 GB free, S3-совместимый, нужен небольшой бэкенд).
- **Без TURN**: звонки только через STUN (на домашнем реле TURN можно
  поднять локально — coturn в compose, но наружу его надо светить
  отдельно, через туннель UDP не пробросить).
- Секреты (`ENCRYPTION_KEY`/`JWT_SECRET_KEY`/`TOTP_MASTER_KEY`) храни
  отдельно: новый релей с новыми ключами = все сессии и 2FA в мусор.

Пул соединений в коде урезан под free-лимиты Supabase (10+20);
крутилки `DATABASE_POOL_SIZE` / `DATABASE_MAX_OVERFLOW` при желании.

## 0.0.1. Релей видит минимум (приватность по умолчанию)

Что оператор релея технически МОЖЕТ увидеть и что с этим сделано:

| Данные | Статус |
|---|---|
| Сообщения после доставки всем | **Удаляются строками** (`RELAY_DEAF=true` по умолчанию; история живёт только на устройствах) |
| Недоставленное старше `MESSAGE_RETENTION_HOURS` | Удаляется (бэкстоп) |
| Кто с кем и когда (граф) | Видит — неустранимо у релея, задокументировано в README |
| Справочник `username → ключи` | Хранится (иначе нет имён и X3DH для офлайн-контактов) — минимум, без которого проводник не работает |
| IP-адреса в БД/аудит-лог | **Не пишет** (`LOG_IPS=false` по умолчанию; rate-limit в памяти работает и без этого) |
| GPS/EXIF в фото | **Срезается при загрузке** (`server/core/storage.py`: transpose + re-save без EXIF, GIF-анимации и файлы без EXIF не трогаем) |
| Содержимое файлов | **Видит** (хранятся открыто, TTL + orphan-cleanup) — настоящий file-E2E (файлы на устройствах) следующий шаг, требует протокола прямой передачи |
| Текст обычных (не-E2E) сообщений | Отклоняется (`RELAY_DEAF` принимает только `encrypted_content`) |

Включить IP-логи обратно (если нужна модерация по IP): `LOG_IPS=true` в `.env`.
...
**Переезд за 10 минут** (триал → триал, триал → дом, дом → триал):

Если БД уже в Supabase — переезда БД вообще нет: на новом хосте
развернуть код, в `.env` тот же `DATABASE_URL` + те же три секрета,
поднять. Аккаунты, история, всё на месте — меняется только адрес,
который друг вводит один раз. Не забудьте `media/` (файлы/аватарки),
иначе сообщения останутся, а вложения побьются.

Если БД ещё локальный SQLite:
...
Аккаунты и история ПЕРЕЕЗЖАЮТ вместе с БД. Перерегистрация не нужна.
...
RELAY_DEAF=true
# База на выбор:
#  а) Нулёвый старт: DATABASE_URL=sqlite:///./nurchat.db (+ бэкап из п.4)
#  б) Данные переживут смерть ПК: DATABASE_URL=<Supabase URI> (см. выше)
```

Вариант (б) строго лучше, если Supabase уже заведён: сдох ПК или кончился
триал — данные целы, поднимаешь релей где угодно с тем же `DATABASE_URL`.
...
**Не подходят для приложения:** Render/HuggingFace free (засыпают;
у HF плюс эфемерный диск), Railway-триалы (кончаются), Fly.io
(фактически только триал + карта), serverless (WebSocket не живёт).
Render убран из схемы полностью — это хостинг для мелких ботов,
не для мессенджера.
