...
**Облачная БД ($0, вечная).** Релей (хоть дома, хоть на триале) ходит
в неё по `DATABASE_URL`. Два варианта:

**A. Supabase free** — 500 MB, без срока. Но: только IPv6.
Если провайдер не даёт IPv6 (проверка: коннект на `[IPv6]:5432`
висит) — Supabase недоступен, бери вариант B.

**B. Прямой адрес без посредников (рекомендуется).** Чужие сервисы
(Tailscale, туннели, облака) — это чужие рубильники: сменят тариф,
закроют free tier, и связь ляжет. Прямое соединение зависит только
от твоего провайдера:

**B1. Белый IP + проброс порта.** Проверь свой IP:
открой `2ip.ru` и сравни с адресом на WAN-порту роутера
(админка роутера → статус). Совпали — IP белый:

1. В роутере: NAT/Port Forwarding → внешний порт `8000` → внутренний
   IP твоего ПК, порт `8000`, TCP.
2. Друг ходит на `http://ТВОЙ_IP:8000`. Всё, больше ничего не надо.
3. IP меняется? У большинства провайдеров белый IP сидит месяцами.
   Если сменился — бесплатный DuckDNS (`duckdns.org`, 2 минуты,
   без карты) + их клиент на ПК обновляет имя сам. Друг ходит на
   `http://имя.duckdns.org:8000` и не замечает смен.

У многих провайдеров белый IP — платная опция ~50 ₽/мес («внешний IP»,
«статический адрес»). Это самые надёжные 50 рублей в схеме.

**B2. Нет белого IP (CGNAT) — Yggdrasil.** Децентрализованная mesh-сеть:
без аккаунтов, без компании, адрес статичный навсегда (привязан к ключу,
`200::/7`), NAT пробивает через публичные пиры:

1. Оба ставите Yggdrasil (`yggdrasil-network.github.io/installation`,
   Windows — next-next-finish, служба стартует сама).
2. У тебя: `yggdrasilctl getSelf` → адрес вида `200:xxxx:...`.
3. Друг в приложении вводит `http://[200:xxxx:...]:8000`, protocol `http`.
   Скобки вокруг IPv6 обязательны!

Минусы честно: +10–30 мс задержки, ещё одна программа на обоих концах.
Зато убить эту связность некому — нет центрального рубильника вообще.

Что через Supabase идёт, а что нет:

| Данные | Где | Видит ли Supabase |
|---|---|---|
| Текст сообщений | Шифротекст в `messages` → удаляется после доставки | Только байты |
| Кому/когда (граф, `chat_id`, `created_at`) | Там же | Да (минимум для маршрутизации) |
| `username → ключи` (справочник) | `users` | Да (иначе нет имён и X3DH) |
| Файлы/фото | `media/` **на диске релея**, не в Supabase | Нет (Supabase их не касается) |
| IP | Нигде (`LOG_IPS=false`) | Нет |
...
| Содержимое файлов | **Не видит** при `is_encrypted=true` (E2E-фрейм `NCF1`, `FILE_E2E_ENABLED=true` по умолчанию; TTL + orphan-cleanup). Открыто лежат только незашифрованные загрузки и аватары |
...
- ПК должен быть включён. Пока релей лежит — отправить нельзя
  (исходящие копятся в шифрованной outbox-очереди на устройстве и уйдут
  сами при появлении связи), прочитать старое — тоже (история на реле).
  Планируй аптайм.
