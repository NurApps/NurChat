...
| Данные | Статус |
|---|---|
| Содержимое E2E-сообщений | Не видит (только `[encrypted]`), при `RELAY_DEAF=true` стирается после доставки |
...
| Содержимое файлов | **Видит** (хранятся открыто) — настоящего file-E2E пока нет, это следующий шаг (roadmap, не обещание) |
| Текст обычных (не-E2E) сообщений | Видит — клиент всегда шлёт `encrypted_content`, если сессия есть |
