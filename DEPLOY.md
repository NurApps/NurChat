...
Supabase здесь — проводник-транзит: через него идут только шифротексты
и маршрутизация (кому/когда), читать ему нечего.

Пошагово (5 минут, карты не надо):
...
2. Слева внизу **Project Settings → Database → Connection string**,
   вкладка **URI**. Копируешь:
   `postgresql://postgres:ТВОЙ_ПАРОЛЬ@db.XXX.supabase.co:5432/postgres`
   Спецсимволы в пароле URL-encode (`@` → `%40`, `#` → `%23`, `?` → `%3F`).
3. В `.env` релея: `DATABASE_URL=<эта строка>`.
4. Проверка:
   ```powershell
   .venv\Scripts\python scripts\supabase-check.py
   ```
   Должен сказать `OK` (или какие таблицы создать через `alembic upgrade head`).
5. Старт релея — таблицы создаются сами при первом запуске.
   Проверка наружу: `curl http(s)://<адрес-релея>/health` → `healthy`.
