...
**Ступень $0-статичный: Render free + cron-job.org анти-сон.**
Единственная бесплатная связка с фиксированным адресом. В репо уже лежит
готовый `render.yaml` — деплой в 3 клика:

1. `dashboard.render.com → New → Blueprint` → подключить репозиторий.
2. Вписать секреты `ENCRYPTION_KEY` / `JWT_SECRET_KEY` / `TOTP_MASTER_KEY`
   (сгенерировать: `python -c "import secrets; print(secrets.token_hex(32))"`)
   **и сохранить их отдельно** — при пересоздании понадобятся те же,
   иначе отвалятся сессии и 2FA.
3. Дождаться деплоя, проверить `https://<имя>.onrender.com/health`.
4. На cron-job.org (бесплатно): задача `GET https://<имя>.onrender.com/health`
   каждые 10 минут — иначе сервис засыпает через 15 минут простоя,
   пробуждение 30–50 секунд (сообщения подождут, звонки в момент
   пробуждения не пройдут).

Честные ограничения ступени (читать до деплоя, не после):
- **Файлы живут только до рестарта.** Диск на Render эфемерный: `media/`
  (фото, файлы, аватары) исчезает при каждом рестарте/редеплое.
  Переписка цела (она в Supabase), вложения — нет. Когда понадобится
  вечное хранение файлов — следующий шаг Supabase Storage (1 GB free,
  S3-совместимый, потребует небольшого бэкенда; пока не сделано).
- **Без TURN** (UDP-диапазоны недоступны): звонки только через STUN.
- Холодный старт 30–50с если пингер пропустил удар.

База переживает всё: Supabase free (500 MB) не имеет срока годности
(в отличие от Render Postgres — 90 дней). Пауза после 7 дней простоя
не страшна: наши пинги `/health` каждые 10 минут держат её тёплой
(health делает `SELECT 1` — это активность). Пул соединений в коде
урезан под free-лимиты (10+20 вместо 20+40); при желании крутится
env `DATABASE_POOL_SIZE` / `DATABASE_MAX_OVERFLOW`.
