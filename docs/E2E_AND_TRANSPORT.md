...
| Реакции (emoji) | ✅ E2E (слепой тег + шифротекст) | relay матчит `tag` (HMAC, необратим) для toggle и хранит `enc_emoji`; emoji видят только участники (группировка после расшифровки на клиенте). Legacy-строки до введения — plaintext, обновляются при повторной реакции |
