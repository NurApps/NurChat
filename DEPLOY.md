...
**B. Neon free (рекомендуется для IPv4-сетей)** — 500 MB, 100 CU-часов/мес,
без карты, IPv4 из коробки, просыпается за ~0.5с после 5 мин тишины:

1. neon.tech → Sign up (GitHub) → **New project** → регион ближе → Create.
2. Dashboard → **Connect** → строка вида:
   `postgresql://user:ПАРОЛЬ@ep-xxxREGION.aws.neon.tech/dbname?sslmode=require`
   Если есть `&channel_binding=require` и коннект падает — убери этот
   параметр, оставь только `sslmode=require`.
3. В `.env` релея: `DATABASE_URL=<эта строка>`.
4. Проверка: `.venv\Scripts\python scripts\supabase-check.py`
   (скрипт зовётся supabase-чеком по истории, проверяет любой Postgres).
5. Старт релея — таблицы создаются сами. Проверка: `/health` → `healthy`.

Supabase-пошагово (если IPv6 есть) — Supabase здесь проводник-транзит:
через него идут только шифротексты и маршрутизация (кому/когда):

1. supabase.com → Sign in (GitHub) → **New project** → имя `nurchat`,
   пароль БД придумай и **запиши** → регион ближе к тебе → Create.
   Ждать ~2 минуты пока статус станет Active.
2. Слева внизу **Project Settings → Database → Connection string**,
   вкладка **URI**. Пароль со спецсимволами можно вставлять как есть —
   приложение кодирует само (и скрипт проверки тоже).
3. В `.env` релея: `DATABASE_URL=<эта строка>`.
4. Проверка — см. п.4 выше.

CU-математика Neon (чтобы не встать посреди месяца): релей 24/7 на
дефолтных интервалах фоновых задач держит compute тёплым всегда —
это ~180 CU-часов, больше лимита 100. Поэтому для Neon в `.env`:

```bash
BACKGROUND_LOOP_SECONDS=600
EPHEMERAL_CLEANUP_SECONDS=600
```

Compute засыпает между прогонами, просыпается по запросу. Доставка
сообщений от цикла не зависит (WebSocket + чтение по запросу) —
только пурдж доставленного опаздывает до 10 минут, это нормально.
